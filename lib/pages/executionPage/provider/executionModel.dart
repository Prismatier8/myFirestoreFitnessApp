...
class ExecutionModel extends ChangeNotifier {
  ...
  Map<String, List<num>> _currentSetMap;

  ///temporary Map to save values from a SetData
  ...
  ///is true when plan execution is finished
  ...
  _getNextExerciseSets() async {
    _currentSetList = [];
    _currentSetIndex = 0;
    final QuerySnapshot snapshot = await _setService
        .getReferencedSetsOrderedBySequence(_exerciseRef[_currentRefIndex]);
    SetModel set;
  ...
  ///Increments the SetIndex and sets the currentSet to the new Set object
  _incrementSetIndex() {
    _currentSetIndex++;
